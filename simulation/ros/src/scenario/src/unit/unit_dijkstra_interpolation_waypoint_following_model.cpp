#include <unit_dijkstra_interpolation_waypoint_following_model.h>
#include <stdexcept>
...
DijkstraInterpolationWaypointFollowingModel::DijkstraInterpolationWaypointFollowingModel()
    : Model()
    , mMovingVehicles{}
    , mMotionConfigs{}
    ...
    , mAgentSpeedManager{}
{
    mEgoVehicleSpeedCommandSubscriber = mNodeHandle.subscribe(
        ...
        &DijkstraInterpolationWaypointFollowingModel::EgoVehicleSpeedCommandCallback,
        this);
    mEgoVehicleGlobalPathSubscriber = mNodeHandle.subscribe(
        "global_path",
        Model::DefaultQueueSize(),
        &DijkstraInterpolationWaypointFollowingModel::EgoVehicleGlobalPathCallback,
        this);
}

std::string DijkstraInterpolationWaypointFollowingModel::GetId() const
{
    return std::string(AgentIdPrefix()) + std::string("model");
}

void DijkstraInterpolationWaypointFollowingModel::Configure(const ModelConfig &config)
{
...
void DijkstraInterpolationWaypointFollowingModel::Update()
{
         ...
         ++movingVehicle, ++motionConfig)
    {
        ...
        if (!moveTriggerPair->second)
        {
            ...
            const motion::MoveTriggerState currentState(
                spatialRelationId,
                egoVehicleToTriggerPointDistance.distance.s());
...
void DijkstraInterpolationWaypointFollowingModel::AccessAgentAttributes(
...
void DijkstraInterpolationWaypointFollowingModel::RunCarlaUpdate(
...
void DijkstraInterpolationWaypointFollowingModel::ConfigureMotions()
{
    Model::ConfigureMotions(mMovingVehicles);

    std::function<decltype(motion::ParseDijkstraInterpolationWaypointFollowingConfig)> parseFunc{
        motion::ParseDijkstraInterpolationWaypointFollowingConfig};
    ...
    if (mMovingVehicles.size() != mMotionConfigs.size())
    {
        ROS_ERROR_STREAM(
            ...
            "mMoviongVehicles: " << mMovingVehicles.size() << '\n' <<
         ...
         ++movingVehicle, ++motionConfig)
    {
            ...
            motionConfig->moveTriggerCondition.triggerRefPosition->GetTypeId())
        {
            ...
            motion::TriggerRefPosition::Type::PositionBased ==
            motionConfig->moveTriggerCondition.triggerRefPosition->GetTypeId())
        {
            moveTrigger = std::make_shared<motion::MoveTrigger>(
...
void DijkstraInterpolationWaypointFollowingModel::EgoVehicleSpeedCommandCallback(
...
void DijkstraInterpolationWaypointFollowingModel::EgoVehicleGlobalPathCallback(
