                             ...
                             uint32_t const laneSectionIndex)
{
  ...
  if (std::size_t{0} == mapData.laneMap.count(currentLaneId))
  {
      std::cerr << __FILE__":" << std::to_string(__LINE__) << '\n' <<
          "invalid currentLaneId: " << currentLaneId << std::endl;
      return;
  }
  auto &laneSections = roadInfo.lanes.lane_sections;
  ...
  else
  {
    if (laneInfo.link->predecessor_id != 0)
    {
      ...
      if (std::size_t{0} == mapData.laneMap.count(predecessorId))
      {
          std::cerr << __FILE__":" << std::to_string(__LINE__) << '\n' <<
              "invalid predecessorId: " << predecessorId << std::endl;
          return;
      }
  ...
  if (laneSectionIndex < laneSections.size())
  {
    if (laneInfo.link->successor_id != 0)
    {
      ...
      if (std::size_t{0} == mapData.laneMap.count(successorId))
      {
          std::cerr << __FILE__":" << std::to_string(__LINE__) << '\n' <<
              "invalid successorId: " << successorId << std::endl;
          return;
      }
