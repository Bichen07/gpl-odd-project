...
{
  const bool isLaneEmpty =
      lane.leftEdge.empty() ||
      lane.rightEdge.empty() ||
      otherLane.leftEdge.empty() ||
      otherLane.rightEdge.empty();
  if (isLaneEmpty)
  {
    std::cout << __FILE__":" << std::to_string(__LINE__) << '\n' <<
        "lane.leftEdge size: " << lane.leftEdge.size() << '\n' <<
        "lane.rightEdge size: " << lane.rightEdge.size() << '\n' <<
        "otherLane.leftEdge size: " << otherLane.leftEdge.size() << '\n' <<
        "otherLane.rightEdeg size: " << otherLane.rightEdge.size() << std::endl;
      return;
  }
