...
def getpoints_np(arr: np.ndarray):
    # Assumes arr.shape = (n, 18)
    # Column indices
    x_i, y_i = arr[:, 0], arr[:, 1]
    hx_i, hy_i = arr[:, 4], arr[:, 5]
    length_i, width_i = arr[:, 7], arr[:, 8]

    x_j, y_j = arr[:, 9], arr[:, 10]
    hx_j, hy_j = arr[:, 13], arr[:, 14]
    length_j, width_j = arr[:, 15], arr[:, 16]

    # vehicle i
    heading_i = np.stack([hx_i, hy_i], axis=1)
    perp_heading_i = np.stack([-hy_i, hx_i], axis=1)
    heading_scale_i = np.linalg.norm(heading_i, axis=1, keepdims=True)

    point_up_i = np.stack([x_i, y_i], axis=1) + (heading_i / heading_scale_i) * (
        length_i[:, None] / 2
    )
    point_down_i = np.stack([x_i, y_i], axis=1) - (heading_i / heading_scale_i) * (
        length_i[:, None] / 2
    )

    point_i1 = (
        point_up_i + (perp_heading_i / heading_scale_i) * (width_i[:, None] / 2)
    ).T
    point_i2 = (
        point_up_i - (perp_heading_i / heading_scale_i) * (width_i[:, None] / 2)
    ).T
    point_i3 = (
        point_down_i + (perp_heading_i / heading_scale_i) * (width_i[:, None] / 2)
    ).T
    point_i4 = (
        point_down_i - (perp_heading_i / heading_scale_i) * (width_i[:, None] / 2)
    ).T
    ...
    # vehicle j
    heading_j = np.stack([hx_j, hy_j], axis=1)
    perp_heading_j = np.stack([-hy_j, hx_j], axis=1)
    heading_scale_j = np.linalg.norm(heading_j, axis=1, keepdims=True)

    point_up_j = np.stack([x_j, y_j], axis=1) + (heading_j / heading_scale_j) * (
        length_j[:, None] / 2
    )
    point_down_j = np.stack([x_j, y_j], axis=1) - (heading_j / heading_scale_j) * (
        length_j[:, None] / 2
    )

    point_j1 = (
        point_up_j + (perp_heading_j / heading_scale_j) * (width_j[:, None] / 2)
    ).T
    point_j2 = (
        point_up_j - (perp_heading_j / heading_scale_j) * (width_j[:, None] / 2)
    ).T
    point_j3 = (
        point_down_j + (perp_heading_j / heading_scale_j) * (width_j[:, None] / 2)
    ).T
    point_j4 = (
        point_down_j - (perp_heading_j / heading_scale_j) * (width_j[:, None] / 2)
    ).T
        ...
        for point_i_start, point_i_end in zip(
            ...
            for point_j_start, point_j_end in zip(
                ...
                if count_i < 2 and count_j < 2:
                    ...
                    dist_mat.append(
                        np.sqrt(
                            (point_i_start[0] - point_j_start[0]) ** 2
                            + (point_i_start[1] - point_j_start[1]) ** 2
                        )
                    )
                    dist_mat.append(
                        np.sqrt(
                            (point_i_start[0] - point_j_end[0]) ** 2
                            + (point_i_start[1] - point_j_end[1]) ** 2
                        )
                    )
                    dist_mat.append(
                        np.sqrt(
                            (point_i_end[0] - point_j_start[0]) ** 2
                            + (point_i_end[1] - point_j_start[1]) ** 2
                        )
                    )
                    dist_mat.append(
                        np.sqrt(
                            (point_i_end[0] - point_j_end[0]) ** 2
                            + (point_i_end[1] - point_j_end[1]) ** 2
                        )
                    )
                ...
                ist = intersect(
                    line(
                        ...
                        + np.array(
                            [
                                -(point_j_start - point_j_end)[1],
                                (point_j_start - point_j_end)[0],
                            ]
                        ),
                    ),
                ...
                dist_mat.append(
                    np.sqrt(
                        (ist[0] - point_i_start[0]) ** 2
                        + (ist[1] - point_i_start[1]) ** 2
                    )
                )

                # Overlapped bounding boxes
                ist = intersect(
                    line(point_i_start, point_i_end), line(point_j_start, point_j_end)
                )
                dist = np.ones(len(samples)) * np.nan
                dist[
                    ison(point_i_start, point_i_end, ist)
                    & ison(point_j_start, point_j_end, ist)
                ] = 0
                dist[
                    np.isnan(ist[0])
                    & (
                        ison(point_i_start, point_i_end, point_j_start)
                        | ison(point_i_start, point_i_end, point_j_end)
                    )
                ] = 0
...
def TTC_ij(samples):
    point_i1, point_i2, point_i3, point_i4, point_j1, point_j2, point_j3, point_j4 = (
        getpoints(samples)
    )
    ...
    for point_line_start in [point_i1, point_i2, point_i3, point_i4]:
        for edge_start, edge_end in zip(
            [point_j1, point_j3, point_j1, point_j2],
            [point_j2, point_j4, point_j3, point_j4],
        ):
            point_line_end = point_line_start + direct_v
            ### intersection point
            ist = intersect(
                line(point_line_start, point_line_end), line(edge_start, edge_end)
            )
            ist[:, ~ison(edge_start, edge_end, ist)] = np.nan
            ### distance from point to intersection point
            dist_ist = np.sqrt(
                (ist[0] - point_line_start[0]) ** 2
                + (ist[1] - point_line_start[1]) ** 2
            )
            dist_ist[np.isnan(dist_ist)] = np.inf
            dist_mat.append(dist_ist)
            leaving = direct_v[0] * (ist[0] - point_line_start[0]) + direct_v[1] * (
                ist[1] - point_line_start[1]
            )
    ...
    TTC = dist2overlap / np.sqrt(
        (samples.vx_i - samples.vx_j) ** 2 + (samples.vy_i - samples.vy_j) ** 2
    )
...
def TTC(samples, toreturn="dataframe"):
    if toreturn != "dataframe" and toreturn != "values":
        ...
        keys = [
            var + "_i" for var in ["x", "y", "vx", "vy", "hx", "hy", "length", "width"]
        ]
        values = [
            var + "_j" for var in ["x", "y", "vx", "vy", "hx", "hy", "length", "width"]
        ]
        keys.extend(values)
        values.extend(keys)
