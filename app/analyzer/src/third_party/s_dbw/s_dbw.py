...
def bind_noise_lab(X, labels, metric):
    ...
    for i in range(len(labels)):
        if labels[i] == -1:
            ...
            lid = np.where(np.all(X == X[labels != -1][np.argmin(dist), :], axis=1))[0][
                0
            ]
            label_id.append(i)
...
def centroid_distance(unique_labels, centroids, metric):
    ...
    for i in unique_labels:
        ...
        for j in unique_labels:
            dist = cdist(
                np.array(centroids[j], ndmin=2),
                np.array(centroids[i], ndmin=2),
                metric=metric,
            )[0, 0]
            module_dist += dist
            if dist > max_dist:
...
def calc_density(
    X, centroids, labels, stdev, clusters_list, method, density_dict=None, lambd=0.7
):
    """
        ...
        if method == "Kim" or method == "Tong":
            ...
            if method == "Tong":
                center_v = lambd * (center_p1 * nj + center_p2 * ni) / nij + (
                    ...
                    (
                        center_p1 * density_dict[clusters_list[0]]
    ...
    for i in unique_labels:
        for j in unique_labels:
            if i == j:
                continue
            result += calc_density(
                X, centroids, labels, stdev, [i, j], method, density_dict
            ) / max(density_dict[i], density_dict[j])
...
def S_Dbw(
    ...
    sdbw = Dens_bw(X, centroids, labels, unique_labels, method) + Scat(
        X, unique_labels, labels, method
    )
    return sdbw
...
def SD(
    ...
    if centers_id:
        ...
        if nearest_centr:
            centroids = calc_nearest_points(X, labels, unique_labels, centroids, metric)
    sd = k * Scat(X, unique_labels, labels, "Halkidi") + centroid_distance(
        unique_labels, centroids, metric
    )
    return sd
