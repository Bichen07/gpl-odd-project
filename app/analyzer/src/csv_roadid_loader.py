...
CACHE_DIR = Path(__file__).resolve().parents[3] / (
    "simulation/ros/.cache/scenario_search/records"
)

_KEEP_COLS = {"time", "name", "x", "y", "h", "roadId", "laneId", "s", "t", "offset", "speed"}
...
def get_csv_road_data(batch_id: int, trial_index: int) -> Optional[pd.DataFrame]:
    ...
    with path.open() as fh:
        ...
        for raw_row in reader:
            row = dict(zip(header, [v.strip() for v in raw_row[:ncols]]))
            # keep only the fields we care about — discard the rest immediately
    ...
    for col in ("time", "x", "y", "h", "speed", "s", "t", "offset"):
