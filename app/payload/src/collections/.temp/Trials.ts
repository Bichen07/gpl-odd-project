...
const getTrajectory = async (
  ...
  const { index, duration, nFrames, trialId, standardized, forward } = query;
  ...
  let startIndex = Math.max(index - durationIndexStep, 0);
  let endIndex = index;
  if (index === -1) {
  ...
  endpoints: [
    ...
    },
    {
      ...
      handler: async (req, res, next) => {
        ...
        const nFrames = Number(req.query["nFrames"]) ?? 10;
