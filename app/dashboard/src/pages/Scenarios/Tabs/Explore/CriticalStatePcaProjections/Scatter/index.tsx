...
export default () => {
  ...
  const clusterInfo = useAppSelector(
    (state) => state.session.selectedCriticalStateClusterInfo,
  );
  ...
  const points: Point[] | null = useMemo(() => {
    ...
    const scores = clusteringResponse.criticalStateClustering.projections;
  ...
  const plotAxes = useMemo(() => {
    let values = Object.values(
      clusteringResponse?.criticalStateClustering.projections ?? {},
    );
        ...
        >
              ...
              ? axes.map((a, index) => {
                ...
                return (
                  <Typography>{`${index === 0 ? "xaxis" : "yaxis"} ${a} (${(
                    (clusteringResponse?.criticalStateClustering
          ...
          >
            {hoveredPoint && clusteringResponse != null
              ? Object.entries(
                clusteringResponse.criticalStateClustering.states[
                  hoveredPoint.trialId
                ].attributes,
              )
                .filter(
                  (attr) =>
