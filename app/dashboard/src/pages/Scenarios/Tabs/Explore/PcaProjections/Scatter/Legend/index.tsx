...
export function Legend({ sx, clusterCounts, mode }: Props) {
  ...
  const selectedCluster = useAppSelector((state) => {
    if (mode === "critical") {
      return state.session.selectedCriticalStateCluster;
    } else {
      return state.session.selectedLastEgoDiffCluster;
    }
  });
      ...
      <Box component="div">
        <Stack>
          <Typography sx={{ fontSize: 14 }}>
            {`${mode === "critical" ? "Critical State" : "Last Ego Diff"}`}{" "}
            Patterns
          </Typography>
          <LegendOrdinal
                  ...
                  return (
                    <>
                      <Box
                        ...
                        onClick={() => {
                          if (label.text === selectedCluster) {
                            dispatch(
                              mode === "critical"
                                ? sessionSlice.actions.setSelectedCriticalStateCluster(
                                  null,
                                )
                                : sessionSlice.actions.setSelectedLastEgoDiffCluster(
                                  null,
                                ),
                            );
                          } else {
                      ...
                      {clusterCounts && label.text in clusterCounts.failed ? (
                        <Typography>{`(${clusterCounts.failed[label.text].size
                          })`}</Typography>
                      ) : null}
                    </>
                  );
