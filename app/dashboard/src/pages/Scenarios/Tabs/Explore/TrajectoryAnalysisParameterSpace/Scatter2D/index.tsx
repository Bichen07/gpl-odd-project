...
export default function Scatter2D({
  ...
  useEffect(() => {
    ...
    if (colorMode === "FpcGradIso") {
      ...
      if (axes === null || Object.keys(parameters).length < 2) {
        return;
      }
      ...
      const layout: Partial<Layout> = {
        ...
        scene: {
          zaxis: { title: "Gradient Magnitude" },
          xaxis: {
            title:
              axes[0] in parameters
                ? `${parameters[axes[0]].name} [${parameters[axes[0]].unit}]`
                : axes[0],
          ...
          yaxis: {
            title:
              axes[1] in parameters
                ? `${parameters[axes[1]].name} [${parameters[axes[1]].unit}]`
                : axes[1],
            showticklabels: false,
            // range:
            //   axes[1] in parameters
      ...
      // const y = gradient.isosurface.map((item) => item.y * 20);
      // const z = gradient.isosurface.map((item) => item.z * 20);
      // const values = gridPreds.map((item) => item.value);
      // const zData = [];
      // for (let i = 0; i < 15; i++) {
      //   const row = [];
      //   for (let j = 0; j < 15; j++) {
      ...
      const layout: Partial<Layout> = {
        ...
        margin: { l: 0, r: 0, t: 0, b: 0, pad: 0 },
        scene: {
          zaxis: { title: "Metric Surface Height" },
          xaxis: {
            title:
              axes[0] in parameters
                ? `${parameters[axes[0]].name} [${parameters[axes[0]].unit}]`
      ...
      for (const trialId in trajectoryAnalysisResponse.trials) {
        const gradient =
          trajectoryAnalysisResponse.clustering.metricGradients[metric.kpi.name]
            .data[trialId];
        if (gradient == null) {
          continue;
        }
      ...
      let chromaScale = chroma.scale("OrRd").padding([0.2, 0]).domain([1, 0]);
      if (metric && metric.kpi.rule === "lessThan") {
        chromaScale = chroma.scale("OrRd").padding([0.2, 0]).domain([0, 1]);
      }
    ...
    const getTitle = (mode: ColorMode) => {
      if (mode === "Cluster") {
        return "Cluster";
      } else if (mode === "FpcGradMag") {
        return "FPC Gradient Magnitude";
      } else if (mode === "MetricGradMag") {
        return `${metric.kpi.name} Gradient Magnitude`;
      } else if (mode === "Metric") {
        return `${metric.kpi.name}`;
      }
    };
      ...
      marker:
        ...
        true
          ? {
            ...
            size: 13,
            opacity: 0.8,
            // opacity: pairedTrialId
            //   ? scatterPlotData.map((i) =>
            //     i.id === pairedTrialId ? 0.8 : 0.1,
            //   )
            //   : 0.8,
          }
          : {
            ...
            colorbar: {
              // title: "TTC Min Gradient Magnitude", // Title of the colorbar
              title: getTitle(colorMode) + " [m]", // Title of the colorbar
    ...
    const layout: Partial<Layout> = {
      ...
      xaxis: {
        title:
          axes[0] in parameters ? `${parameters[axes[0]].name} [m]` : axes[0],
        range:
          axes[0] in parameters
            ? [parameters[axes[0]].min, parameters[axes[0]].max]
            : undefined,
  ...
  useEffect(() => {
    const element = document.getElementById("scatter-2d-root");
    if (element) {
