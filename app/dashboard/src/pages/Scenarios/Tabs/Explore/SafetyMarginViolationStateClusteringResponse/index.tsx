...
export default function SafetyMarginViolationStateClusteringResponse() {
  ...
  const clusterInfos = useAppSelector(
    (state) => state.session.safetyMarginViolationStateClusterInfos,
  );
  ...
  const results =
    useAppSelector(
      (state) =>
        state.session.clusteringResponse?.safetyMarginViolationStateClustering
          .results,
    ) ?? [];
      ...
      <CardContent sx={{ height: "100%" }}>
        <Stack rowGap={1} sx={{ overflowY: "scroll", height: "100%" }}>
          ...
          {sortedIndices?.map((index) => {
            ...
            return (
              <Stack
                key={`${result.task.method}_${result.task.nClusters}`}
                onClick={() => {
                  dispatch(
                    sessionSlice.actions.setSelectedSafetyMarginViolationStateClusteringResult(
                      clusteringResponse.safetyMarginViolationStateClustering
                        .results[index] ?? null,
                    ),
                  );
                  dispatch(
                    sessionSlice.actions.setSelectedSafetyMarginViolationStateClusterInfo(
