...
| How to interpret clusters? | `build_llm_dataset.sh` → dashboard **Select and analyze** (or `run_cluster_analyze.sh --results-dir …`) |
...
Generated by `scripts/build_llm_dataset.sh` and the analyze flow (dashboard **Select and analyze** / `scripts/run_cluster_analyze.sh`):
...
```bash
python3 scripts/generate_map_tracks.py --batch-id 1   # → results/map/hct_6_tracks.csv (+ copies hct_6.xodr)
python3 scripts/map_preprocess.py --batch-id 1        # → results/map/hct_6.{yaml,jpg,_description.txt}
```
...
bash scripts/build_llm_dataset.sh --source payload-save --batch-id 1 --k 4 --silhouette 0.7037
...
| `hct_6.xodr` | Sim cache | `generate_map_tracks.py`, `map_preprocess.py` | OpenDRIVE map |
...
| Artifact | Producer | Consumers | Content |
|----------|----------|-----------|---------|
| `hct_6_tracks.csv` | `scripts/generate_map_tracks.py` | `tier2_renderer` / `MapPlotter` | odrplot lane geometry for BEV |
| `hct_6.xodr` (copy) | `scripts/generate_map_tracks.py` | `labeller`, parser | local copy of the map |
| `hct_6.yaml` | `scripts/map_preprocess.py` | `labeller` (`JunctionRoads`) | junction-road list for turn/junction detection |
| `hct_6.jpg`, `hct_6_description.txt` | `scripts/map_preprocess.py` | optional LLM map context | full-network reference render |
...
folder produced by `build_llm_dataset.sh`) and, for every `cluster<N>/`, asks a multimodal
...
4. build_llm_dataset.sh --source payload-save --batch-id 1 --k <k>
...
| **3** | Build cluster artifacts | `bash scripts/build_llm_dataset.sh --source payload-save --batch-id 1 --k 4` |
